package br.com.base.modelo;

import java.util.InputMismatchException;

import br.com.base.excessoes.CpfException;

public class Cpf extends Documento {


	public Cpf(String cpf) throws CpfException {
		this.isCPF(cpf);
		super.setNumero(cpf);

	}

	public void isCPF(String CPF) throws CpfException {
		// considera-se erro CPF's formados por uma sequencia de numeros iguais
		if (CPF.equals("00000000000") || CPF.equals("11111111111")
				|| CPF.equals("22222222222") || CPF.equals("33333333333")
				|| CPF.equals("44444444444") || CPF.equals("55555555555")
				|| CPF.equals("66666666666") || CPF.equals("77777777777")
				|| CPF.equals("88888888888") || CPF.equals("99999999999")
				|| (CPF.length() != 11))
			throw new CpfException(
					"Cpf's formados por uma sequencia de numeros iguais");

		char dig10, dig11;
		int sm, i, r, num, peso;

		// "try" - protege o codigo para eventuais erros de conversao de tipo
		// (int)
		try {
			// Calculo do 1o. Digito Verificador
			sm = 0;
			peso = 10;
			for (i = 0; i < 9; i++) {
				// converte o i-esimo caractere do CPF em um numero:
				// por exemplo, transforma o caractere '0' no inteiro 0
				// (48 eh a posicao de '0' na tabela ASCII)
				num = (int) (CPF.charAt(i) - 48);
				sm = sm + (num * peso);
				peso = peso - 1;
			}

			r = 11 - (sm % 11);
			if ((r == 10) || (r == 11))
				dig10 = '0';
			else
				dig10 = (char) (r + 48); // converte no respectivo caractere
											// numerico

			// Calculo do 2o. Digito Verificador
			sm = 0;
			peso = 11;
			for (i = 0; i < 10; i++) {
				num = (int) (CPF.charAt(i) - 48);
				sm = sm + (num * peso);
				peso = peso - 1;
			}

			r = 11 - (sm % 11);
			if ((r == 10) || (r == 11))
				dig11 = '0';
			else
				dig11 = (char) (r + 48);

			// Verifica se os digitos calculados conferem com os digitos
			// informados.
			if (!(dig10 == CPF.charAt(9)) && (dig11 == CPF.charAt(10)))
				throw new CpfException(
						"Digitos calculados conferem com os digitos informados");

		} catch (InputMismatchException erro) {
			throw new CpfException("Erros de conversao de tipo");

		}
	}

	public String grtCpfFormatado() {
		return (this.getNumero().substring(0, 3) + "." + this.getNumero().substring(3, 6) + "."
				+ this.getNumero().substring(6, 9) + "-" + this.getNumero().substring(9, 11));
	}

	@Override
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + ((getNumero() == null) ? 0 : getNumero().hashCode());
		return result;
	}

	@Override
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (obj == null)
			return false;
		if (getClass() != obj.getClass())
			return false;
		Cpf other = (Cpf) obj;
		if (getNumero() == null) {
			if (other.getNumero() != null)
				return false;
		} else if (!getNumero().equals(other.getNumero()))
			return false;
		return true;
	}

}
